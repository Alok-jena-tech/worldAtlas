...
const CountryDetails = () => {
  ...
  console.log("cou data", country);
  // console.log("yyyyyyy",country.name.nativeName)
  // console.log("kkkk",Object.keys(country.name.nativeName))
  useEffect(() => {
    startTransition(async () => {
      const res = await getCountryIndData(param.id);
      if (res.status === 200) {
        console.log("oppppooo");
        // setCountry(res.data[0]);
        console.log(res.data[0], "kkkkkkkkk");
            ...
            <div className="country-content">
              ...
              <div className="infoContainer">
                ...
                <p><span className="card-description">Currencies:</span>{
                  Object.keys(country.currencies).map((a)=>
                   country.currencies[a].name
                  ).join(" , ")
                }</p>
                <p><span className="card-description">languages:</span>{
                  Object.keys(country.languages).map((a)=>country.languages[a]).join(",")
                }</p>
              </div>
            </div>
          </div>
